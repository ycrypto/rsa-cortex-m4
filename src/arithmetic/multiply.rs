...
mod test {
    ...
    fn multiply() {
        ...
        assert_ne!(wrapping_mul(&p, &q).to_unsigned::<8,8>().unwrap(), Unsigned::<8,8>::try_from_slice(&n).unwrap());
        assert_eq!(wrapping_mul(&p, &q).to_unsigned::<4,4>().unwrap(), Unsigned::<4,4>::from_slice(&n[..(n.len()/2)]));

        assert_eq!(&*n.to_bytes(),
